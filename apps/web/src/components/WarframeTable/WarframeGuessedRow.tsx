...
export interface WarframeWithMatch extends WarframeDto {
  correct?: boolean;
  fieldMatches?: {
    name?: boolean;
    type?: boolean;
...
export function WarframeGuessedRow({ guessedWarframes, columns }: WarframeGuessedRowProps) {
    const getCellClass = (match?: boolean | string) => {
      if (match === true || match === "equal") return "bg-teal-700 text-white";
      if (match === false || match === "lower" || match === "higher") return "bg-red-800 text-white opacity-80";
      return "";
    };
    ...
    return (
        ...
        {guessedWarframes.map((w) => (
                ...
                >
                  ...
                  {columns
                    .filter((col) => col.key !== "wikiaThumbnail")
                    .map((col) => {
                      
                      const match = (w.fieldMatches as any)?.[col.key];
                      ...
                      return (
                        <TableCell
                          key={col.key}
                          className={`px-4 py-2 text-center ${getCellClass(match)}`}
                        >
                          {displayValue}
                          {col.key === "releaseYear" && getYearIcon(match)}
                        </TableCell>
                      );
                    })}
                </TableRow>
